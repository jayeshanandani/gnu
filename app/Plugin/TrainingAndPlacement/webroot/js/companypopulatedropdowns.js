$(function() {
    function getData(selectedValue, targetUrl, destination) {
        $.ajax({
            ...
            success: function(response) {
                if (response.company_jobs) {
					destination.empty(),
					destination.append('<option value="Please Select">Please Select</option>');
					appendData(response.company_jobs, destination); // Another from duch with the appendData function added as well, but not working either.
                }
	...
	function appendData(data, destination) {
     ...
     $('#company_masters').change(function() {
        ...
        destination.empty(),
        destination.append('<option value="Select Department First">Select Company First</option>');
        }
    });
    });
